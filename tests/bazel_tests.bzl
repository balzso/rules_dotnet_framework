load("@rules_dotnet_framework//dotnet/private:common.bzl", "env_execute")
load("@rules_dotnet_framework//dotnet:defs.bzl", "dotnet_context")
...
_env_build_template = """
load("@rules_dotnet_framework//tests:bazel_tests.bzl", "bazel_test_settings")
bazel_test_settings(
  name = "settings",
  bazel = "{bazel}",
  exec_root = "{exec_root}",
...
_bazel_test_script = rule(
    ...
    attrs = {
        ...
        "dotnet_context_data": attr.label(default = Label("@rules_dotnet_framework//:dotnet_context_data")),
    },
...
def bazel_test(name, command = None, args = None, targets = None, dotnet_version = None, tags = [], externals = [], workspace = "", build = "", check = "", config = None, workspace_in = None, build_in = None, srcs = None):
    ...
    externals = externals + [
        "@rules_dotnet_framework//:AUTHORS",
    ]
    ...
    native.sh_test(
        ...
        data = [
            "@bazel_test//:bazelrc",
            "@rules_dotnet_framework//tests:rules_dotnet_deps",
...
md5_sum = rule(
    ...
    attrs = {
        ...
        "_md5sum": attr.label(
            executable = True,
            default = "@rules_dotnet_framework//dotnet/tools/md5sum",
            cfg = "host",
        ),
    },
)
