load("@rules_dotnet_framework//dotnet/private:context.bzl", "core_context_data", "dotnet_context_data", "net_context_data")
load("@rules_dotnet_framework//dotnet:defs.bzl", "DEFAULT_DOTNET_CORE_FRAMEWORK", "DOTNET_CORE_FRAMEWORKS", "DOTNET_NET_FRAMEWORKS")
...
[
    core_context_data(
        ...
        csc = "@rules_dotnet_framework//dotnet/stdlib.core/{}:csc.dll".format(framework),
        framework = framework,
        libVersion = DOTNET_CORE_FRAMEWORKS[framework][1],
        runner = "@core_sdk_{}//:runner".format(framework),
        runtime = "@rules_dotnet_framework//dotnet/stdlib.core/{}:runtime".format(framework),
        visibility = ["//visibility:public"],
    )
